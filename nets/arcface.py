...
class Arcface(nn.Module):
    def __init__(self, num_classes=None, backbone="iresnet50", pretrained=False, mode="predict"):
        super(Arcface, self).__init__()
        # set net
        """
        这里的 self.arcface 是主干网络。
        """
        ...
        # set mode type
        self.mode = mode
        if mode == "train":  # 但实际上不会用到，这个架构下只是 predict
    ...
    def forward(self, x, y=None, mode="predict"):
        ...
        if mode == "predict":
            ...
            x = self.head(x, y)  # todo 这里是 arcface head 的作用点位
            return x

    def freeze(self, layers_num=0):
        self.arcface.freeze(layers_num)
